...
function ExchangeCnpjRow({ exchange }: { exchange: Exchange }) {
  ...
  return (
    <li className="flex flex-wrap items-start justify-between gap-3 py-3">
      <span className="pt-6 text-sm text-gray-900">{exchange.name}</span>
      <div className="flex flex-wrap items-end gap-2">
        ...
        {dirty && (
          <button
            ...
            onClick={handleSave}
            className="cursor-pointer rounded-md border border-gray-300 px-2 py-1 text-xs text-gray-700 hover:bg-gray-100 disabled:cursor-not-allowed disabled:opacity-50"
          >
      ...
      {mutation.isError && <p className="w-full text-xs text-red-600">{saveErrorMessage(mutation.error)}</p>}
    </li>
  )
}
...
function CoinGrupo08Row({ coin }: { coin: Coin }) {
  ...
  return (
    <li className="flex flex-wrap items-start justify-between gap-3 py-3">
      <span className="pt-6 text-sm text-gray-900">
        {coin.symbol} <span className="text-gray-500">{coin.name}</span>
      </span>
      <div className="flex flex-wrap items-end gap-2">
        <div className="flex flex-col gap-1">
          <label htmlFor={inputId} className="text-xs font-medium text-gray-500">
    ...
    <details className="rounded-lg border border-gray-200 p-4" open>
      ...
      <div className="mt-4 space-y-6">
        <div>
          ...
          {exchangesLoading && <p className="mt-2 text-sm text-gray-400">Carregando...</p>}
          {!exchangesLoading && (
        ...
        <div>
          <h3 className="text-sm font-medium text-gray-900">Moedas</h3>
          {coinsLoading && <p className="mt-2 text-sm text-gray-400">Carregando...</p>}
          {!coinsLoading && (
            <ul className="mt-1 divide-y divide-gray-100">
              {coins?.map((coin) => (
